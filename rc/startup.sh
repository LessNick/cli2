loadfont -s /system/res/fonts/8x8/default.fnt
;type /system/res/docs/logo.txt
;echo \x10\x06[CLi\xfd] Command Line Interface II for WildCommander v0.18
;echo \x10\x072012,2015 \x7f Breeze\\Fishbone Crew
;echo
;echo \x10\x0dType \x14 help \x14 to display the full list of commands.
echo  ______ _____   _______ ______ 
echo |      |     | |_     _|__    | \x10\x06[CLi\xfd] Command Line Interface II v0.18
echo |   |__|     |_ _|   |_ __|   | \x10\x072012,2015 \x7f Breeze\\Fishbone Crew
echo |      |       |       |    __|
echo |______|_______|_______|______| \x10\x0dType \x14 help \x14 to display the full list of commands
echo \x10\x10
;keyscan
;nvram -s #f0,#02 -g #f0 -g #f0 -g #f0 -g #f0
;loadsxg testpal.sxg
;screenfx -fi
;sleep 0
;screenfx -fo
;screen 0
;sh /demo/bramble/run.sh
;loadsxg /test.sxg
;cd /demo/bramble/art/
;pwd


