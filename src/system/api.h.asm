...
;---------------------------------------
; cliKernel		equ	#7003			; Точка входа в API
...
printWithScroll		equ	#20			; Спрашивать ли «Scroll?» при печати 30 строк за раз
...
setFont			equ	#34			; Установить шрифт для текстового режима
...
printSymbolRepeat 	equ	#3D			; Вывести один символ до тех пор пока позиция X не будет равна заданному числу
							; i: A' - код символа ASCII
							;    C  - позиция X до которой повторять символ

eatSpaces		equ	#3E			; Удалить лишние пробелы перед словом
							; i: DE - адрес начала строки для проверки
							; o: DE - адрес начала строки

setAppCallBack		equ	#3F			; Установить адрес вызова подпрограммы при переключении Alt+F1/F2/F3/F4
							; i: HL - адрес подпрограммы

setRamPage0		equ	#40			; Включить указанную банку по адресу #0000
							; i: A' - номер банка

getRamAlloc		equ	#41			; Возвращает номер банки памяти, в которой расположены данные
							; i: A' - ID данных:
...
moveScreenInit		equ	#42			; Инициализировать графический экран в 0,0
...
nvRamSetData		equ	#4D			; Записать данные в ячейки nvRam
							...
							; #08 – регистр месяца
							; #09 – регистр года (смещение в очень хитром формате)
...
getKeyWithShift		equ	#51			; Получить ASCII код нажатой клавиши:
							; o: A - ASCII код нажатой клавиши (включая коды спецклавиши из ascii_keys.h.asm)

waitAnyKey		equ	#52			; Ожидание нажатия любой клавиши

; 			equ	#53			; зарезервировано!
;			equ	#54			; зарезервировано!
...
setMouseCursor		equ	#58			; Установить внешний вид курсора мыши
							; i: A' - внешний вид:
...
enableAyPlay		equ	#59			; Разрешить воспроизведение AY. Данные должны быть предварительно
							; загружены в ayBank и проинициализированы драйвером с помощью pt3init

disableAyPlay		equ	#5A			; Запретить воспроизведение AY. При этом вызывается драйвер - pt3mute

uploadAyModule		equ	#5B			; Установить (скопировать) модуль в ayBank
							; i: HL - адрес откуда будут копирваться данные.
							;         адрес должен быть не меньше #4000.
							;    BC - размер модуля

setPalNow		equ	#5C			; Изменить палитру для текущего активного графического (текстового)
...
showHideCursor		equ	#5D			; Разрешить/Запретить отображение курсора мыши на экране
							; i:B - номер графического экрана (1,2,3-GFX)
							;   C - #00 скрыть, #01 отображать

setCurrentInk		equ	#5E			; Установить текущий цвет (Ink) печати
							; i:A' - номер цвета из палитры (0-15)

getCurrentInk		equ	#5F			; Получить текущий цвет (Ink) печати
							; o: A - номер цвета из палитры (0-15)

setCurrentPaper		equ	#60			; Установить текущий цвет (Paper) печати
							; i:A' - номер цвета из палитры (0-15)

getCurrentPaper		equ	#61			; Получить текущий цвет (Paper) печати
							; o: A - номер цвета из палитры (0-15)

long2str		equ	#62			; Преобразовать 32-битное число в строку с десятичным значением
							; i: DE - значение (старшая часть)
							;    HL - значение (младшая часть)
							;    DE' - адрес начала строки (10 символов)

getCurrentDate		equ	#63			; Получить текущую дату
							; o:HL - год
							;    D - месяц
							;    E - день
							;    C - день недели

getCurrentTime		equ	#64			; Получить текущее время
							; o:H - часы
							;   L - минуты
							;   С - секунды

getMonthNameByNumber	equ	#65			; Получить 3-х буквенное назавание месяца по номеру
							; i: A - номер месяца (1-12)
							;    DE - адрес строки куда сохранить 3 символа

getDayNameByNumber	equ	#66			; Получить 3-х буквенное назавание дня недели по номеру
							; i: A - номер дня (1-7)
							;    DE - адрес строки куда сохранить 3 символа
