...
;---------------------------------------
colorInk		db	cWhite				; 0й - основной цвет чернил
colorPaper		db	cCobaltBlue			; 1й - основной цвет бумаги
colorBorder		db	cCobaltBlue			; 2й - основной цвет бордера

colorError		db	cRed				; 3й - цвет сообщения об ошибке
colorWarning		db	cOrange				; 4й - цвет сообщения предупреждения
colorOk			db	cGreen				; 5й - цвет сообщения об успехе

colorHelp		db	cFrostySky			; 6й - цвет сообщения подсказки

colorDir		db	cWhite				; 7й - цвет аттрибута - директория
colorFile		db	cLightViolet			; 8й - цвет аттрибута - файл
colorRO			db	cAmigaPink			; 9й - цвет аттрибута - только для чтения
colorHidden		db	cFrostySky			;10й - цвет аттрибута - скрытый
colorSystem		db	cOrange				;11й - цвет аттрибута - системный
colorArch		db	cGreen				;12й - цвет аттрибута - архивный

colorAppName		db	cAmigaPink			;13й - цвет вывода названия приложения
colorCopyright		db	cLightViolet			;14й - цвет вывода информации о владельце (©)

colorInfo		db	cWhiteAluminum			;15й - цвет инфомационного сообщения

colorDefault		db	cWhite				;16й - цвет по умолчанию

colorFrames		db	cLemonCream			;17й - цвет рамки
;---------------------------------------
...
printX			db	#00				; позиция X для печати в буфере256
strLen			db	#00
...
;---------------------------------------
; Цвета мерцания курсора
;---------------------------------------
curAnimPos		db	#00
curAnim			db	14,cWhite			; timeout,color
			db	 5,cWhiteAluminum		; timeout,color
			db	 7,cCobaltBlue			; timeout,color
			db	 5,cWhiteAluminum		; timeout,color
			db	#00

cursorType		db	"_"				; внешний вид курсора (_-ovr,|-ins)
...
tmpPrintStr		ds	255,#00				; Временая строка для печати различной информации (в частности вывод Help)
...
;-------------------------------------------------------------------------------------
; Таблица занятости памяти различными данными:
;-------------------------------------------------------------------------------------
memAllocData		db	video0Bank			;
			db	video1Bank			;
			db	video2Bank			;
			db	video3Bank			;
;-------------------------------------------------------------------------------------
