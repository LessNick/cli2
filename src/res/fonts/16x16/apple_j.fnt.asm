...
sFont		db	#7f,"FNT"				; #7f+"FNT" - 4 байта сигнатура, что это формат файла FNT
		...
		db	#02					; 1 байт формат данных шрифта:
								;		#01 - 1 bit (обычный ч/б) шрифт
								;		#02 - 4 bit 16-ти цветный шрифт
								;		#03 - 8 bit 256-ти цветный шрифт
		dw	#0010					; 2 байта ширина шрифта
		dw	#0010					; 2 байта высота шрифта
		dw	bFont-taFont				; 2 байта смещение от текущего адреса до начала данных шрифта

taFont		dw	bpFont-tsFont				; 2 байта смещение от текущего адреса до начала палитры шрифта (если шрифт не 1bit)
tsFont		dw	#0000					; 2 байта смещение от текущего адреса до начала таблицы ширины шрифта (если шрифт пропорциональный)

								; Мета-данные:
		dw	neFont-nFont				; 2 байта длина название шрифта
nFont		db	"Apple Jack Font",#00			; * байт название шрифта, оканчивающихся кодом #00
neFont		
		dw	aeFont-aFont				; 2 байта длина автора шрифта
aFont		db	"breeze/fishbone",#00				; * байт автор шрифта, оканчивающихся кодом #00
aeFont		
		dw	deFont-dFont				; 2 байта длина описания шрифта
dFont		db	"Font dedicated Apple Jack from MLP",#00	; * байт описание шрифта, оканчивающихся кодом #00
deFont

bpFont		incbin  "rc/fonts/16x16/apple_jack.pal"		; Начало данных палитры шрифта
epFont

;		ds	256,0					; таблица ширины символов для пропорционального шрифта
								; если задан bit 7 в типе шрифта

bFont		incbin  "rc/fonts/16x16/apple_jack.spr"		; Начало данных шрифта
eFont


; 	DISPLAY "bFont-taFont",/A,bFont-taFont

	SAVEBIN "install/system/res/fonts/16x16/apple_j.fnt", sFont, eFont-sFont
